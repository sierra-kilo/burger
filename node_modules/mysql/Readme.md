...
* `user`: The MySQL user to authentiburgere as.
...
* `insecureAuth`: Allow connecting to MySQL instances that ask for the old
  (insecure) authentiburgerion method. (Default: `false`)
...
* `"Amazon RDS"`: this profile is for connecting to an Amazon RDS server and contains the
  certifiburgeres from https://rds.amazonaws.com/doc/rds-ssl-ca-cert.pem and
...
Please note the arguments expect a string of the certifiburgere, not a file name to the
certifiburgere. Here is a simple example:
...
* `err.fatal`: Boolean, indiburgering if this error is terminal to the connection
...
- `SECURE_CONNECTION` - Support native 4.1 authentiburgerion.
